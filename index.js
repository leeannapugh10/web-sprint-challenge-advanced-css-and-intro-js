...
/* Task 1: Practice accessing data above by 
console.log-ing following items:

(1) Name of the first artist (0th index) 
in the array

(2) Bio of the third artist (2nd index) 
in the array */
...
/* Task 2: There is a typo in your dataset 
😱 The 9th artist, Vincent Van Gogh is 
currently Vincent Van Dough. Use an array 
method to fix this issue and console.log() 
to check your work. */

function fixName(arry) {
  artists[8].name = "Vincent Van Gogh";
}
fixName(artists)
console.log(artists[8].name)

/* Task 3: Create a function called 
`getArtistByIndex` that takes two arguments:
 *     (1) artists array
 *     (2) a number which is the desired 
 * index in the array.
 * getArtistByIndex returns a string in 
 * the format `The artist at index {id} 
 * is {name}.`
 * 
 * For example, if getArtistByIndex is 
 * invoked with the artists dataset and the
 *  number 0,
 * it will return `The artist at index 0 
 * is Amedeo Modigliani`.
*/
...
an array with names of artists who were born
 in and died in 20th century (1900-2000) 
 example born in 1901 and died in 1959 - 
 included / born in 1889 and died in 1925 
 not included - should return ["Salvador 
 Dali", "Frida Kahlo"]*/

function get20s(arry){
  arry2 = [];
    for (let i = 0; i < arry.length; i++) {
      arry2.push (arry[i].years.split(" - "));
    }
    let years = [];
      for (let k = 0; k < arry2.length; k++) {
        if ((arry2[k][0]) >= 1900 && (arry2[k][1]) <= 2000) {
          years.push(arry[k].name)
        }
      }
    return years;
  }
...
/* Task 5: Create a function called 
`removeArtist` that takes two arguments:
 *     (1) artists array
 *     (2) a number which is the desired 
 * index in the array.
 * removeArtist removes an artist from the
 *  array at the index and console.logs the 
 * length of the remaining dataset.
 * 
 * For example, if removeArtist is invoked
 *  with the data and the number 0,
 * it will remove Amedeo Modigliani from our
 *  dataset and log the number 19. 
 * 
 * Note that sucessfully invoking this 
 * function multiple times without refreshing
 *  your browser will continuously remove 
 * artists from the array until there 
 * are none left. If you refresh your 
 * browser, the data will reset.  
*/
...
/**
/* Task 6: Create a function called 
`addArtist` that can accept an object of 
information and add it to the artists array. 
Then, add a 21st artist to the array (you) 
...
At the end, this function should return the
 new array with information added"*/

function addArtist(id, name, years, genre, nationality, bio){
    const newArtist = {"id": id, "Name":name, "Years":years, "Genre":genre, "Nationality":nationality, "bio":bio}
    artists.push(newArtist);
    return artists;
  }
console.log (addArtist (20, "Leeanna Pugh", "1991-Current", "Web Design", "American", "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Pharetra diam sit amet nisl suscipit adipiscing bibendum."))

/* Task 7: Create a function called 
lotsOfArt() that takes one argument: 

    (1) artists array 

and returns an array with names of artists
 who painted more than 100 paintings.

For example lotsOfArt(artists); will return 
["Amedeo Modigliani", "Rene Magritte", 
..."Albrecht Dürer"]*/
...
/* STRETCH 1: Programmatically console.log
 HTML element structure.

In HTML, every artist and associated content
...
Create a function called `getHTML()` 
that takes the parameter `data` and uses a
 for..in loop to format and console.log data
  like the example above. 

The function should console.log 50 chunks 
of HTML code that match the structure above. 

‼️ You do **NOT** need to get these to display
 on your page, but you can copy and paste 
 the result into your HTML file if you'd
  like to see what that would look like. */
...
/* STRETCH 2: Create a function called
 `randomize` that takes a data array as an 
 argument and returns a the same array in a
  randomized order. */
...
function randomize(/* Code here */){
 ...
 /* STRETCH 3: Use advanced array methods 
 (.map, .reduce, .filer) to refactor your 
 MVP code (create an array of all artists 
  born in the 1900s with .filter, for example)
   */
